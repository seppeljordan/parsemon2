...
def test_fmap_can_replace_parsing_result(runner):
    parser = fmap(lambda x: "b", literal("a"))
...
def test_failure_of_choice_of_3_literals_should_contain_all_3_literal():
    p = choice(literal("first"), choice(literal("second"), literal("third")))
    with pytest.raises(ParsingFailed) as err:
...
def test_seperated_by_empty(runner):
    assert runner(seperated_by(literal("a"), literal(",")), "").value == []


def test_seperated_by_one_element(runner):
    assert runner(seperated_by(literal("a"), literal(",")), "a").value == ["a"]


def test_seperated_by_five_elemts(runner):
    assert runner(seperated_by(literal("a"), literal(",")), "a,a,a,a,a").value == [
        "a",
        "a",
        "a",
        "a",
...
def test_seperated_by_1000_elements(runner):
    assert (
        runner(seperated_by(literal("a"), literal(",")), "a" + ",a" * 999).value
        == ["a"] * 1000
    )


def test_enclosed_by(runner):
    assert runner(enclosed_by(literal("a"), literal('"')), '"a"').value == "a"


def test_choices(runner):
    assert runner(choices(literal("a"), literal("b"), literal("c")), "b").value == "b"
...
def test_that_fmap_does_not_change_error_messages(runner):
    ...
    assert str(error_message_without_fmap.value) == str(error_message_with_fmap.value)
